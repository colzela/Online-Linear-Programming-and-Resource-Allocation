...
p = np.array([i+1 for i in range(m)])      # ground truth price vector: m*1
A = np.random.randint(0, high = 2, size = (m, n))
A = A.astype('float64')

pi = np.matmul(p, A) + random.gauss(0, math.sqrt(0.2))
...
for i in range(n-1):
    # a += np.round(np.random.randn(m)+1)
    pi_d += random.gauss(2,1)
    # A_dependent.append(a.copy())
    Pi_dependent.append(pi_d)
